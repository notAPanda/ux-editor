...
<script>
import { styler, translate, rotate } from 'free-transform'
import { getSineCosine, getPoint, getOppositePoint } from '@/helpers/point-finder'
...
const scale = (scaleType, {
  ...
  return (event) => {
    ...
    const rotatedMoveDiff = {
      x: ((moveDiff.x * cos) - (moveDiff.y * sin)),
      y: ((moveDiff.x * sin) + (moveDiff.y * cos))
    }
    switch (scaleType) {
      case 'ml':
